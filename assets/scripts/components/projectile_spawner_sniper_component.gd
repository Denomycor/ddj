class_name ProjectileSpawnerSniperComponent extends Node2D


signal shoot_projectile(from: Vector2, rot: float, data: Variant)
signal projectile_ready

@export var fire_delay: float = 2.0  # Disparo lento
@export var multishot: int = 1  # Apenas um projétil por vez
@export var spread: float = 0.0  # Sem variação no ângulo de disparo
